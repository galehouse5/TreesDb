...
}}}(function(a){var l="PLACEHOLDER-INPUT";
var m="PLACEHOLDER-LABEL";
var c=false;
var g={labelClass:"placeholder"};
var h=document.createElement("input");
if("placeholder" in h){a.fn.placeholder=a.fn.unplaceholder=function(){};
delete h;
return
}delete h;
a(window).resize(d);
a.fn.placeholder=function(o){b();
var p=a.extend(g,o);
this.each(function(){var s=Math.random().toString(32).replace(/\./,""),q=a(this),r=a('<label style="position:absolute;display:none;top:0;left:0;"></label>');
if(!q.attr("placeholder")||q.data(l)===l){return
}if(!q.attr("id")){q.attr("id","input_"+s)
}r.attr("id",q.attr("id")+"_placeholder").data(l,"#"+q.attr("id")).attr("for",q.attr("id")).addClass(p.labelClass).addClass(p.labelClass+"-for-"+this.tagName.toLowerCase()).addClass(m).text(q.attr("placeholder"));
q.data(m,"#"+r.attr("id")).data(l,l).addClass(l).after(r);
k.call(this);
i.call(this)
})
};
a.fn.unplaceholder=function(){this.each(function(){var o=a(this),p=a(o.data(m));
if(o.data(l)!==l){return
}p.remove();
o.removeData(l).removeData(m).removeClass(l).unbind("change",j)
})
};
function b(){if(c){return
}a("form").live("reset",function(){a(this).find("."+l).each(i)
});
a("."+l).live("keydown",k).live("mousedown",k).live("mouseup",k).live("mouseclick",k).live("focus",k).live("focusin",k).live("blur",i).live("focusout",i).live("change",j);
a("."+m).live("click",function(){a(a(this).data(l)).focus()
}).live("mouseup",function(){a(a(this).data(l)).focus()
});
bound=true;
c=true
}function j(){var o=a(this);
if(!!o.val()){a(o.data(m)).hide();
return
}if(o.data(l+"FOCUSED")!=1){n(o)
}}function k(){a(a(this).data(l+"FOCUSED",1).data(m)).hide()
}function i(){var o=this;
n(a(this).removeData(l+"FOCUSED"));
setTimeout(function(){var p=a(o);
if(p.data(l+"FOCUSED")!=1){n(p)
}},200)
}function n(p,o){var q=a(p.data(m));
if((o||q.css("display")=="none")&&!p.val()){q.text(p.attr("placeholder")).css("top",p.position().top+"px").css("left",p.position().left+"px").css("display","block")
}}var f;
function d(){if(f){window.clearTimeout(f)
}f=window.setTimeout(e,50)
}function e(){a("."+l).each(function(){var p=a(this);
var o=a(this).data(l+"FOCUSED");
if(!o){n(p,true)
}})
}}(jQuery));